...
export default function MLModelling() {
  ...
  const startML = async () => {
    if (!targetColumn) {
      setStartError(
        "No target column found. Run EDA first from the Dashboard.",
      );
      return;
    }
    setStarting(true);
    setStartError("");
    try {
      const res = await api.post("/api/run_ml", {
        filename,
    ...
    <div>
      ...
      {!isRunning && !result && (
        <div className="mt-8 surface p-8 text-center">
          ...
          <p className="text-sm text-[var(--muted)] font-['JetBrains_Mono'] mb-6">
            The agent will select the best models, train them, and evaluate
            feature importance.
          </p>
          {startError && (
      ...
      {ml?.status === "failed" && (
        <>
          ...
          <div className="mt-4 flex justify-center">
            <button
              onClick={startML}
              className="rounded-full bg-[var(--blue)] px-6 py-2.5 text-sm font-semibold text-[var(--bg)]"
            >
              Retry →
            </button>
          </div>
      ...
      {result && (
        <>
          ...
          {result.feature_plan && (
            <Card title="Feature Engineering Plan" className="mt-6">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <Group
                  label="Dropped"
                  items={result.feature_plan.drop}
                  color="text-red-400"
                />
                <Group
                  label="Numeric"
                  items={result.feature_plan.numeric}
                  color="text-[var(--yellow)]"
                />
                <Group
                  label="One-Hot"
                  items={result.feature_plan.onehot}
                  color="text-emerald-400"
                />
                <Group
                  label="Ordinal"
                  items={Object.keys(result.feature_plan.ordinal || {})}
            ...
            <Card title="Model Comparison" className="mt-4">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-[var(--muted)] font-['JetBrains_Mono'] text-xs">
                  ...
                  <tbody>
                    {Object.entries(result.results).map(([name, data]) => (
                      <tr
                        key={name}
                        className={`border-t border-[var(--line)] ${name === result.best_model ? "bg-[var(--surface-2)]" : ""}`}
                      >
                        <td className="py-2 pr-4 font-semibold">
                          {name}
                          {name === result.best_model && (
                            <span className="ml-2 rounded bg-[var(--blue)] px-1.5 py-0.5 text-[9px] font-bold text-[var(--bg)]">
                              BEST
                            </span>
                          )}
                        </td>
                        {result.problem_type === "classification" ? (
                          <>
                            <td className="py-2 pr-4 font-['JetBrains_Mono']">
                            ...
                            <td className="py-2 font-['JetBrains_Mono']">
                              {data.metrics.recall}
                            </td>
                          </>
                        ) : (
          ...
          {result.results?.[result.best_model]?.feature_importance && (
            <Card
              title={`Feature Importance — ${result.best_model}`}
              className="mt-4"
            >
              {Object.entries(
                result.results[result.best_model].feature_importance,
                  ...
                  return (
                    <div key={feat} className="mb-3">
                      <div className="flex justify-between text-xs mb-1">
                        <span>{feat}</span>
                        <span className="text-[var(--yellow)] font-['JetBrains_Mono']">
                          {val}
                        </span>
                      </div>
                      <div className="h-1.5 w-full rounded-full bg-[var(--surface-2)] overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.round((Math.abs(val) / maxVal) * 100)}%`,
                          }}
                        />
                      </div>
                    </div>
          ...
          <div className="mt-6 flex justify-center">
            <button
              onClick={startML}
              className="rounded-full border border-[var(--line)] px-6 py-2.5 text-sm font-semibold hover:border-[var(--line)] transition-colors"
            >
              Retrain →
            </button>
          </div>
...
const Tag = ({ label, accent }) => (
  <span
    className={`rounded-full px-3 py-1.5 text-xs font-semibold ${accent ? "bg-[var(--blue)] text-[var(--bg)]" : "border border-[var(--line)] text-[var(--muted)]"}`}
  >
    {label}
  </span>
);
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {items.map((i) => (
          <span
            key={i}
            className="rounded bg-[var(--surface-2)] px-2 py-1 text-[11px] text-[var(--muted)]"
          >
            {i}
          </span>
        ))}
...
const Spinner = () => (
  <div className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--line)] border-t-[#c97539]" />
);
