...
export default function DataEDA() {
  ...
  return (
    <div>
      ...
      {!isRunning && !charts && hasEdaJob && (
        <StatusBar label="Waiting for results…" />
      )}
          ...
          {visibleCount < chartEntries.length && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                className="rounded-full border border-[var(--line)] px-7 py-3 text-sm font-semibold hover:border-[var(--line)] transition-colors"
              >
                Show next{" "}
                {Math.min(PAGE_SIZE, chartEntries.length - visibleCount)} →
              </button>
            </div>
          )}
        </>
  ...
  return (
    <div className="surface flex flex-col overflow-hidden">
      ...
      <div className="border-t border-[var(--line)] p-4">
        ...
        {err && (
          <p className="mt-2 text-xs text-red-400 font-['JetBrains_Mono']">
            ✗ {err}
          </p>
        )}
        {analysis && (
          <div className="mt-4 space-y-3 text-sm">
            {analysis.represents && (
              <p className="text-[var(--text)]">{analysis.represents}</p>
            )}
...
const List = ({ label, items, color }) => (
  <div>
    ...
    <ul className="space-y-1 text-[var(--muted)] text-xs list-disc list-inside">
      {items.map((item, i) => (
        <li key={i}>{item}</li>
      ))}
    </ul>
...
const Spinner = () => (
  <div className="h-4 w-4 animate-spin rounded-full border-2 border-[var(--line)] border-[var(--blue)]" />
);
