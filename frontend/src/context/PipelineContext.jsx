...
function readJobs() {
  try { return JSON.parse(localStorage.getItem("pipelineJobs") || "null"); }
  catch { return null; }
}
...
export function PipelineProvider({ children }) {
  const [filename, setFilename] = useState(() => localStorage.getItem("selectedDataset") || null);
  ...
  const setterFor = (key) => (key === "info" ? setInfo : key === "eda" ? setEda : setMl);
  ...
  useEffect(() => {
    ...
    steps.forEach((s) => { doneRef.current[s.key] = false; });

    const pollOne = async (s) => {
      const jobId = readJobs()?.[s.idKey];
      if (!jobId) { doneRef.current[s.key] = true; return; }
      try {
    ...
    const tick = async () => {
      await Promise.all(steps.filter((s) => !doneRef.current[s.key]).map(pollOne));
      if (allDone()) stop();
    };
    ...
    (async () => {
      ...
      intervalRef.current = setInterval(async () => {
        if (++attempts > 120 || !readJobs()) { stop(); return; }
        await tick();
      }, 3000);
    })();
  ...
  const addJob = useCallback((key, jobId) => {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (DONE.includes(r.data.status)) { doneRef.current[shortKey] = true; stop(); }
      } catch (e) {
        if (e.response?.status === 404) {
          dropStaleJob(key);
  ...
  const clearJobs = useCallback(() => {
    ...
    setInfo(null); setEda(null); setMl(null);
  ...
  const loadInsights = useCallback(async (fname) => {
    ...
    try {
      ...
      setStoredInsights((prev) => ({ ...prev, [fname]: res.data.insights || null }));
    } catch (e) {
  ...
  const value = {
    info, eda, ml, polling, gone, filename,
    addJob, clearJobs, selectDataset,
